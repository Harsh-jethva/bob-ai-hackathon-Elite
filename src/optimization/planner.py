...
def forecast_buckets(model, live, inbound, ports, now):
    ...
    for pid, cfg in ports.items():
        nb = len(cfg["berths"])
        rows, hist = [], [live[pid]["waiting_now"]] * 5
        for b in range(HORIZON_HOURS // BUCKET_HOURS):
            t = now + timedelta(hours=b * BUCKET_HOURS)
            win6 = inbound[(inbound.port == pid) & (inbound.eta >= t)
                           & (inbound.eta < t + timedelta(hours=6))]
            win12 = inbound[(inbound.port == pid) & (inbound.eta >= t)
                            & (inbound.eta < t + timedelta(hours=12))]
            f = pd.DataFrame([dict(
                vessels_waiting_now=hist[-1],
                berth_occupancy_pct=live[pid]["berth_occupancy_pct"],
                crane_util_pct=live[pid]["crane_util_pct"],
                yard_occupancy_pct=live[pid]["yard_occupancy_pct"],
...
def build_plan(now, live, inbound, model, ports, diversion_cost, crane_rate=CRANE_RATE_TEU_PER_H):
    # 1) PREDICT congestion for the next 72h per port (requirement #1)
    ...
    if len(recs):
        div = recs.set_index("vessel_id").to_port.to_dict()
        delay = recs.set_index("vessel_id").extra_steaming_h.to_dict()
        for vid in div:  # diverted ships arrive later (extra steaming time)
            inbound.loc[inbound.vessel_id == vid, "eta"] += pd.Timedelta(hours=delay.get(vid, 0))
    else:
        div = {}
    ...
    for pid, cfg in ports.items():
        ...
        base = solve_greedy(todo, cfg, now, crane_rate=crane_rate, occupied=occupied)
        if opt is not None and len(opt):
            opt["port"] = pid
            sched_all.append(opt)
    ...
    # 4) KPIs (requirement #4 - the dashboard layer)
    kpis = dict(
